#ifndef osx_SKILL_SERVER_H
#define osx_SKILL_SERVER_H
...
#include "osx_helper_functions.h"
...
#include "osx_msgs/RobotStatus.h"

// Services
#include "osx_msgs/sendScriptToUR.h"
#include "osx_msgs/publishMarker.h"
...
public:
  ...
  bool publishMarkerCallback(osx_msgs::publishMarker::Request &req,
  ...
  // Camera multiplexer callbacks
  void cameraMultiplexerCallback(const dynamic_reconfigure::Config::ConstPtr &msg);

  bool activateCamera(const std::string camera_name);
  std::string getCurrentActiveCamera();

  // Public variables
  bool a_bot_ros_control_active_ = false;
  bool b_bot_ros_control_active_ = false;
...
private:
  ...
  std::map<std::string, osx_msgs::RobotStatus> robot_statuses_;
  ...
  // Camera multiplexer related members
  std::string current_active_camera_;
  std::vector<std::string> camera_names_;
  ros::Subscriber camera_multiplexer_sub_;
  boost::mutex camera_mutex_;
  bool camera_names_initialized_ = false;
}; // End of class SkillServer

#endif // osx_SKILL_SERVER_H
