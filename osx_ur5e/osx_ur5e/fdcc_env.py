...
class FDCCEnv(BaseEnv):
    ...
    def __init__(self, config: DictConfig, node=None, runtime=None, use_torch_for_cameras=False):
        super().__init__(
            ...
            node=node,
            runtime=runtime,
            use_torch_for_cameras=use_torch_for_cameras,
        )
    ...
    def load_params(self, config):
        ...
        self.node.get_logger().info(f"Cameras to record from: {list(self.cam_names)}")
        ...
        self.translation_scale = float(getattr(config.controller, "translation_scale", 1.0))
        self.rotation_scale = float(getattr(config.controller, "rotation_scale", 1.0))
    ...
    def set_controller_parameters(self):
        ...
        self.arm.set_solver_parameters(
            error_scale=error_scale, iterations=iterations, publish_state_feedback=True
        )
    ...
    def prepare_action(self, action):
        if "action.contact_direction" in action:
            fvt_action = process_factored_action_dict(
                action,
        ...
        elif "action.position" in action and "action.orientation" in action:
    ...
    def set_compliant_control_action(self, action):
        ...
        now = time.monotonic()
        if now - self.last_stiffness_command_stamp > 0.025 and not np.all(
            np.isclose(self.last_stiffness_params, stiff_act, atol=5.0)
        ):
            self.arm.update_stiffness(stiff_act)
            self.last_stiffness_params = np.copy(stiff_act)
            self.last_stiffness_command_stamp = now
        ...
        if self.actions_as_deltas:
            assert len(action_rotation) == 3, (
                "Rotation actions must be 3D for delta actions"
            )
            delta_translation, delta_orientation = self.clip_delta_actions(
                action_translation * self.translation_scale,
                action_rotation * self.rotation_scale,
            )
        else:
            assert len(action_rotation) == 4, (
                "Rotation actions must be quaternion (4D) for absolute actions"
            )
            delta_translation, delta_orientation = self.clip_delta_actions(
                action_translation - current_pose[:3],
                transformations.quaternions_orientation_error(
                    action_rotation, current_pose[3:]
                ),
            )

        target_position = current_pose[:3] + delta_translation
        target_orientation = transformations.rotate_quaternion_by_rpy(
            *delta_orientation, current_pose[3:]
        )
    ...
    def clip_delta_actions(self, delta_translation, delta_orientation):
        clipped_delta_translation = np.clip(
            delta_translation, -self.max_delta_translation, self.max_delta_translation
        )
        clipped_delta_orientation = np.clip(
            delta_orientation, -self.max_delta_rotation, self.max_delta_rotation
        )
